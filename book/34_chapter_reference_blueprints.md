...
![Figure 34.1 — Blueprint selection decision tree: regulatory intensity, zone count, connectivity model, and workload classification as primary selectors](images/figure-34-1.png)
...
![Figure 34.2 — Single-sovereign-zone topology: regions, tiers, networking, and observability data flows within a single zone boundary](images/figure-34-2.png)
...
![Figure 34.3 — Multi-zone, multi-cloud topology: zone boundaries, inter-zone connectivity, federated observability, and the Concert unified topology layer](images/figure-34-3.png)
...
Concert's role in the multi-zone blueprint is elevated compared to the single-zone case. It becomes the unified topology layer that holds the cross-zone dependency graph: which services in zone A depend on APIs provided by services in zone B, which business capabilities span multiple zones, and which zones are affected when a shared dependency degrades (Chapter 14). This cross-zone topology is assembled from the zone-level topology exports that each zone's discovery agents produce, and it enables Concert to generate cross-zone situation analysis and impact assessment without accessing zone-internal data.
...
![Figure 34.4 — Air-gapped topology: the self-contained sovereign perimeter, internal service layout, and the controlled import/export boundary](images/figure-34-4.png)
...
![Figure 34.5 — Hybrid sovereign-commercial topology: sovereign zone(s) and commercial tier with boundary management, workload placement policies, and unified operations view](images/figure-34-5.png)
...
The policy framework is hierarchical. Organisation-wide policies—data residency constraints, encryption requirements, tagging standards—apply to all zones and all tiers. Zone-specific policies—approved regions, approved providers, zone-specific access controls—apply within individual zones. Application-specific policies—custom compliance requirements for particular regulatory regimes—apply to specific workloads. The hierarchy is enforced by the policy engine: zone-specific policies cannot weaken organisation-wide policies, and application-specific policies cannot weaken zone-specific policies. Chapter 10 describes the continuous compliance monitoring that validates this hierarchy at runtime.
...
In multi-zone deployments, identity federation must respect zone boundaries. A human operator authenticated in zone A does not automatically have access to zone B; they must be separately authorised under zone B's access policies. Service accounts in one zone do not have credentials valid in another zone unless an explicit cross-zone trust relationship has been established and approved. The identity framework enforces these boundaries while providing a unified directory that allows the central operations team to manage identity lifecycle—onboarding, role changes, offboarding—through a single process that propagates to all zones (Chapter 13).
