...
![Figure 5.1 — Sovereign zone topology: primary, auxiliary and edge zones across providers](images/figure-5-1.png)
>
> A topology diagram showing a primary sovereign zone (customer on‑premises or Satellite location, with dedicated OpenShift cluster, local key vault and OPA/Gatekeeper enforcement), connected via private links to an auxiliary zone (hyperscaler VPC, multi‑tenant cluster, namespace segmentation) and an edge zone (on‑premises site, single‑purpose cluster, local policy enforcement with periodic synchronisation to hub).
...
![Figure 5.2 — Multi‑cloud topology patterns: hub‑and‑spoke, provider‑aligned rings, and edge‑anchored mesh](images/figure-5-2.png)
>
> Three topology diagrams arranged horizontally: (left) a hub‑and‑spoke layout with a central RHACM hub cluster connected to three spoke OpenShift clusters in different sovereign zones; (centre) a provider‑aligned ring with OpenShift clusters at AWS, Azure and IBM Cloud linked by private interconnects; (right) an edge‑anchored mesh with a central hub cluster, auxiliary cloud clusters and multiple edge clusters at factory and retail sites, with synchronisation arrows indicating intermittent connectivity.
...
The analysis in this chapter has implicitly assumed that the estate's integration model supports the topology's sovereignty goals. That assumption is the subject of Chapter 6. Zero‑copy integration, as the operational substrate on which sovereign services run, determines whether the access paths, event flows and data boundaries designed into the topology are genuinely honoured at runtime. A sovereign topology built on a copy‑heavy integration estate is, ultimately, a leaky vessel: the data will find its way to where it should not be, not through deliberate breach but through the accumulated gravity of poorly governed pipelines. Chapter 6 examines how zero‑copy principles harden the integration layer against that drift, and how the combination of principled topology and zero‑copy substrate creates the coherent, demonstrably sovereign estate that both regulators and the organisation's own risk appetite demand.
